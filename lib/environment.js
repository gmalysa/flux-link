...
function Environment(env, log) {
	...
	// Copy over reference object
	for (key in env) {
		this[key] = env[key];
	}
	this._fm = new FluxMeta(this, log);

	// Bind member methods so that they can be passed as callbacks
	this.$throw = ep.$throw.bind(this);
	this.$catch = ep.$catch.bind(this);
}

// Shorthand for accessing the environment prototype
var ep = Environment.prototype;
...
ep.$throw = function(err) {
	...
	if (h === undefined) {
		...
		this._fm.$log('Stack trace: ' + err.stack);
	}
	else {
 ...
 */
ep.$format_call_tree = function(bt) {
 ...
 */
ep.$format_stack_trace = function(bt) {
	return this._fm.$format_stack_trace(bt);
}

/**
 * A scoped environment is a wrapper whose local scope is separate from that of the environment,
 * but it can see everything originally defined in the environment (and also modify it). This
 * allows for chains to declare "local" variables that are unique to each running instance of
 * the chain and also disappear automatically after it ends, avoiding polluting the environment
 */
ep.createScoped = function() {
