...
_.extend(ChainBase.prototype, {
	 ...
	 */
	make_after_glue : function(env, after, except) {
		var that = this;
...
_.extend(ChainManager.prototype, {
	queueTick : function(fn, args) {
		args = args || this.noargs;
		if (this.queue[this.next]) {
			this.queue[this.next].fn = fn;
			this.queue[this.next].args = args;
		}
		else
			this.queue[this.next] = {fn : fn, args : args};
		this.next += 1;
	...
	runTick : function() {
		var stop = this.next;
		for (var idx = 0; idx < this.next; ++idx) {
			this.queue[idx].fn.apply(null, this.queue[idx].args);
			this.queue[idx].fn = this.nullfn;
			this.queue[idx].args = this.noargs
		}
		this.next = 0;
		this.tickSet = false;
	}
